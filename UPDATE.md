...
## v1.4.1 更新内容（2026-08-19）

### 新增：香港假期标记视觉区分

香港"休"标记改为**正圆圈 + 主题色**（`--calendar-primary`，默认紫色），大陆"休"保持**方框 + 周末红**。形状与颜色双重区分：
...
实现：`updateGrid` 渲染标记时，当天命中 `hkFestival` 则追加 `calendar-holiday-hk` 修饰类；CSS 定义正圆（等宽高 + flex 居中 + `border-radius: 50%`）并使用主题色。
